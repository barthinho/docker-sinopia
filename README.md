## Sinopia (Docker Image) based on https://github.com/mhart/alpine-node
...
`docker run --name sinopia -d -p 4873:4873 barthinho/sinopia:latest`
...
docker run -v /path/to/config.yaml:/opt/sinopia/config.yaml --name sinopia -d -p 4873:4873 barthinho/sinopia:latest
```
